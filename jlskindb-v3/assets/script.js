...
(function () {
    ...
    async function loadTranslations() {
        ...
        // Parse title/device/resolution from the filename, but let an explicit
        // title from tl/index.json (via tl/titles.json) win when present.
        T.all = files.map(f => {
            const parsed = parseJarName(f.name);
            return { ...parsed, ...f, title: f.title || parsed.title };
        });
        T.loaded = true;
        T.loading = false;
